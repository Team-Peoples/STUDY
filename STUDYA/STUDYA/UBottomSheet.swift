...
//  STUDYA
//
//  Created by OfTheWolf and contributors (https://github.com/OfTheWolf/UBottomSheet)
//
...
//    🛑EHD: library overriding
    public func appearTwice(_ position: CGFloat, animated: Bool, completion: @escaping () -> ()) {
        ...
        if animated {
            ...
            dataSource.animator?.animate(animations: {
                ...
                self.setPosition(self.availableHeight * 0.6, animated: true)
            })
        } else {
            self.container!.frame = frame
            self.delegate?.bottomSheet(self.container,
