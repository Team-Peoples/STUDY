...
//        switch textField {
//        case emailTextField:
//            emailInputView.changeSeparatorColor(into: UIColor.appColor(.defaultGray))   //ehd: 씬에 진입하면 이메일 텍스트필드가 firstresponder되면서 색깔바뀌는건 잘 되는데 바로 이메일 입력안하고 패스워드텍스트필드를 한번 눌렀다가 다시 돌아오면 색이 안바뀌고 애초에 이메일 입력 없이 패스워드로 바로 갈 때도 패스워드 텍스트필드의 seaparator가 색이 바뀌지 않음
//            return true
//        case passwordTextField:
